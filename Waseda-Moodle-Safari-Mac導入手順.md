...
1. `waseda-moodle-login-helper-safari-1.2.0.zip`
...
ditto -x -k ~/Downloads/waseda-moodle-login-helper-safari-1.2.0.zip ~/Developer/WasedaMoodleAutoLogin/extension
ls ~/Developer/WasedaMoodleAutoLogin/extension
```

最後のコマンドで`manifest.json`，`core.js`，`microsoft.js`，`moodle.js`などが表示されることを確認する。

ZIPを別の場所へ保存した場合は，`~/Downloads/waseda-moodle-login-helper-safari-1.2.0.zip`を実際のパスへ置き換える。
